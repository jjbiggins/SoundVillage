...
export default class PlaylistDetailScreen extends React.Component {
    ...
    onQueuedCollectionUpdate = (querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
            const { songID, upvotes, downvotes, score } = doc.data();
    ...
    async componentDidMount() {
      ...
      //  let playlistPrivacy = firebase.firestore().collection("playlists").doc(playlistID).get();
        ...
        this.unsubscribeDequeued = this.ref.doc(playlistID).collection('dequeuedSongs').orderBy('timeAdded', 'desc').onSnapshot(this.onDequeuedCollectionUpdate);
    ...
    */
    ...
    renderMenu() {
        if (this.state.type == 'admin' || this.state.isPrivate){
            return (
                <View style={{flexDirection: 'row', width: '90%', alignItems: 'center', justifyContent: 'center', margin: 10}}>
                    <Button
                        type="clear"
                        title="View Playlist Info"
                        titleStyle={{color: '#303338'}}
            ...
            return (
                <View style={{flexDirection: 'row', width: '90%', alignItems: 'center', justifyContent: 'center', margin: 10}}>
                    ...
                    <Button
                        type="clear"
                        title="Un-Save Playlist"
                        containerStyle={{margin: 5}}
                        titleStyle={{color: '#303338'}}
                        onPress={() => this.unsavePlaylist()}
                    />
                </View>
            )
        }
    ...
    renderBanner() {
        if (Dimensions.get('window').height < 670) {
            return (
                <LinearGradient start={{x: 0, y:0}} end={{x: 0, y:1}} colors={['#53687E', '#F7F7F2']} style={styles.topView1}>
                    <Text adjustsFontSizeToFit={true} numberOfLines={1} style={{color: '#F7F7F2', fontSize: 30, fontWeight: 'bold', fontFamily:'coolvetica', marginTop: 5, marginRight: 5, marginLeft: 5}}>
                        {this.state.playlist.name}
                    </Text>
                    <View style={{margin:3, width: "60%", height:'95%', borderRadius: 7, flex: 1, alignItems: 'center'}}>
                        <View style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center'}}>
                            {this.renderAvatar()}
                            <Text style={{color: '#303338', fontSize: 14}}>
        ...
        return (
            <LinearGradient start={{x: 0, y:0}} end={{x: 0, y:1}} colors={['#53687E', '#F7F7F2']} style={styles.topView}>
            ...
            <SafeAreaView style={styles.container} forceInset={{ bottom: 'never' }}>
                ...
                <View style={{bottom: 0, top: 0}}>
                    <View style={{height:Dimensions.get('window').height * 0.25, marginRight: 10, marginLeft: 10, bottom: 0, marginBottom: 10}}>
                        <Text style={{fontSize: 25, fontWeight: 'bold', fontFamily: 'coolvetica', color: '#303338'}}>Queue</Text>
                        <FlatList
                    ...
                    <View style={{top: 0, height: Dimensions.get('window').height * 0.25, margin: 10, right: 0, left: 0}}>
                        <Text style={{fontSize: 25, fontWeight: 'bold', fontFamily: 'coolvetica', color: '#303338'}}>Songs Played</Text>
